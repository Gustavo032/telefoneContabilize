	...
	useEffect(()=>{
	  ...
	  <Box maxW="7xl" mx={'auto'} pt={5} px={{ base: 2, sm: 12, md: 17 }}>
		...
		<SimpleGrid columns={{ base: 1, md: 3 }} spacing={{ base: 5, lg: 8 }}>
		  <StatsCard hoje={false} title={`Ontem (${atendeOntem.quando})`} stat={`${atendeOntem.quem}`} />
		  <StatsCard hoje={true} title={`Hoje (${atendeHoje.quando})`} stat={`${atendeHoje.quem}`} />
		  <StatsCard hoje={false} title={`Amanhã (${atendeAmanha.quando})`} stat={`${atendeAmanha.quem}`} />
		</SimpleGrid>
	  </Box>
	);
}